...
public final class MyArrayListDouble{
	public double[] ARRAY;
	...
	public MyArrayListDouble(int initialSize){
		ARRAY = new double[initialSize];
		capacity = initialSize;
	}
	public MyArrayListDouble(double[] array){
		ARRAY = array;
	 ...
	 * @param to_copy
	 */
	public MyArrayListDouble(MyArrayListDouble to_copy){
		ARRAY = new double[to_copy.ARRAY.length];
	...
	public void add(int element){
		...
		if(writeHere==capacity){//esure capacity if full
			double[] newArray = new double[2*capacity];
	...
	public void ensureCapacity(final int requiredCapacity) {
		if(requiredCapacity>this.capacity){
			double[] newArray = new double[2*requiredCapacity];
			System.arraycopy(ARRAY, 0, newArray, 0, capacity);
			capacity *= 2;
			this.ARRAY = newArray;
		}
	 ...
	 * @return
	 */
	int compare(MyArrayListDouble otherList){
	...
	public void add(final MyArrayListDouble toAdd){
		final int NEW_SIZE = this.size()+toAdd.size();
		ensureCapacity(NEW_SIZE);
		double[] from = toAdd.ARRAY;
		double[] to = this.ARRAY;
	...
	public void intersect(MyArrayListDouble toIntersect){
		final int cand_length 		= this.size();
		final int to_merge_length 	= toIntersect.size();
		final double[] candidates 	= this.ARRAY;
		final double[] toMerge 	= toIntersect.ARRAY;
...
*/
	...
	public final boolean isIn(final int value) {
		final int size = size();
		final double[] raw_array = this.ARRAY;
