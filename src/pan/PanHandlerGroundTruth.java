...
public class PanHandlerGroundTruth extends DefaultHandler {
	    ...
	    public void startElement(String uri, String lName, String qName, Attributes attr) throws SAXException {
	        switch (qName) {
	            case FEATURE:
	            	...
	            	String temp;
	            	Detection current = new Detection();
	            	current.susp = reference;
	            	//current.this_offset = attr.getQName(0);
	            	temp = attr.getValue(1+2);
	            	current.this_offset   = Integer.parseInt(temp);
	            	current.this_length   = Integer.parseInt(attr.getValue(2+2));
	            	current.src			  = attr.getValue(3+2);
	            	current.source_offset = Integer.parseInt(attr.getValue(4+3));
	            	current.source_length = Integer.parseInt(attr.getValue(5+3));
